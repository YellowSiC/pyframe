...
pub fn get_js_code(
    py_api: Option<bool>,
    socketio_settings: Option<crate::options::SocketSettings>,
) -> anyhow::Result<String> {
    let mut js_code = r#"(function () {
  ...
  (function docReady(func) {
    ...
    // Falls SocketIO initialisiert werden soll:
    if let (Some(true), Some(settings)) = (py_api, socketio_settings) {
        let settings_json = serde_json::to_string(&settings)?;
        let socket_code = format!(
            r#"

(function () {{
  if (typeof io !== 'undefined') {{
    window.PyFrame.socket = io({}, {});
    console.log('Socket.IO initialized with settings: {}');
  }} else {{
    console.error('Socket.IO not found!');
  }}
}})();"#,
            settings_json,
            "{}", // Platzhalter für transportOptions falls nötig
            settings_json
        );
        js_code.push_str(&socket_code);
    }
